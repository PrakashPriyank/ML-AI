...
def TrackImages():
    ...
    i = 0
    j = 0
    recognizer = cv2.LBPHFaceRecognizer_create()#cv2.face.LBPHFaceRecognizer_create()
    ...
    while True:
        ...
        #resized_img = cv2.resize(im, (1000, 700))
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                aa1 = df.loc[df['SERIAL NO.'] == serial]['ADDRESS'].values
            ...
            cv2.putText(im, str(bb)+"  "+str(aa1), (x, y + h), font, 1, (255, 255, 255), 2)
            cv2.putText(im, predicted_emotion, (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
            cv2.putText(im, f'{gender}, {age}', (faceBox[0], faceBox[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2, cv2.LINE_AA)
        #cv2.imshow("Detecting age and gender", resultImg)
        cv2.imshow('Multi Face Recognition System', im)
        """if(k==0):
            k=1
        else:
            k=0
